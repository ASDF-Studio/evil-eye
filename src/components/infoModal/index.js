...
const InfoModal = ({ isvisible, onClose, children }) => {
    ...
    return (
        <FlexCenter className={`z-50 ml-5 sm:ml-0 fixed right-10 top-24 bg-black bg-opacity-25 backdrop-blur-sm shadow-sm transition-transform ${modalVisible ? 'transform translate-x-0 bounce' : `transform translate-x-[120%] ${shouldAnimateClose ? 'closeAnimation' : ''}`}`} id='wrapper' onClick={handleClose}>
