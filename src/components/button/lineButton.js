...
export const LineButton = ({
  ...
  const buttonMapping = {
    ...
    primary: "w-56 h-[45px] bg-main-sky5 rounded-xl shadow shadow-buttonShadow",
    secondary:
  ...
  return (
    <button
      className={[
        buttonMapping[variant],
        "flex justify-center items-center gap-2",
      ].join(" ")}
      {...rest}
    >
      <div>
        {leftIcon ? <div className="">{leftIcon && leftIcon}</div> : null}
      </div>
      <div className={["", className].join(" ")}>
        <h3 className="text-center font-roman text-[35px] font-normal tracking-[-3.5px] leading-[32px] uppercase drop-shadow-3xl hover:tracking-[-4px]">
          {children}
        </h3>
      ...
      <div>{endIcon ? <div className="">{endIcon && endIcon}</div> : null}</div>
    </button>
  );
};
